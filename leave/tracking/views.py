...
from .forms import SignUpForm, LeaveRequestForm, LeaveApprovalForm, ProfileForm
...
def leave_history(request):
    ...
    # Filter leave requests by status (using lowercase as defined in model)
    pending_requests = leave_requests.filter(status='pending')
    approved_requests = leave_requests.filter(status='approved')
    rejected_requests = leave_requests.filter(status='rejected')

    return render(request, 'leave_history.html', {
        'leave_requests': leave_requests,  # Main queryset for displaying all requests
        'pending_requests': pending_requests,
        'approved_requests': approved_requests,
        'rejected_requests': rejected_requests,
    })
...
def profile_view(request):
    ...
    total_approved_days = sum([leave.days_count for leave in approved_leaves])
    ...
    context = {
        ...
        'recent_requests': leave_requests[:5],
    }
